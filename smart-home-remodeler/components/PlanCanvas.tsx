...
import { FloorPlanData, Wall, Point, RemodelZone, ScaleData, RulerData } from '../types';

interface PlanCanvasProps {
  imageSrc: string | null;
  imageDims: { width: number; height: number } | null;
  data: FloorPlanData | null;
  mode: 'VIEW' | 'CALIBRATE' | 'ZONE' | 'EDIT';
  scaleData: ScaleData;
...
const PlanCanvas: React.FC<PlanCanvasProps> = ({
  ...
  // Calculate dynamic styling based on plan size to ensure visibility
  const planWidth = imageDims?.width || 1000;
  const planHeight = imageDims?.height || 1000;
  const uiScale = Math.max(planWidth, planHeight) / 1000; // Base scale on 1000px

  // Dynamic visual properties
  const wallStroke = 5 * uiScale;
  const wallStrokeExt = 8 * uiScale;
  const handleRadius = 15 * uiScale; // Larger handles for easier grabbing
  const rulerStroke = 4 * uiScale;
  const fontSize = 24 * uiScale;


  // Robust Coordinate conversion using SVG Matrix
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    if (mode === 'CALIBRATE') {
       // Check if clicking ruler handles
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (mode === 'ZONE' && dragStart) {
      ...
      // Normalize rect (handle negative drag)
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-slate-100 border rounded-lg shadow-inner cursor-crosshair">
      ...
      <div className="absolute top-4 left-4 bg-white/90 p-2 rounded shadow text-xs space-y-1 backdrop-blur-sm pointer-events-none z-10">
        <div className="flex items-center gap-2"><div className="w-3 h-3 bg-red-500 rounded-full"></div> Structural Anchor (External)</div>
        <div className="flex items-center gap-2"><div className="w-3 h-3 bg-slate-800 rounded-full"></div> Internal Wall</div>
        <div className="flex items-center gap-2"><div className="w-3 h-3 bg-green-500/50 border border-green-500 rounded"></div> Remodel Zone</div>
        {scaleData.calibrated && (
            <div className="mt-2 text-slate-500 font-mono">Scale: 1ft = {scaleData.pixelsPerFoot.toFixed(2)}px</div>
        )}
      </div>
    </div>
  );
