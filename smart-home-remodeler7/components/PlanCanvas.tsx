...
import { FloorPlanData, Wall, Point, RemodelZone, ScaleData, RulerData, Rect, OrientationData } from '../types';

interface PlanCanvasProps {
  imageSrc: string | null;
  imageDims: { width: number; height: number } | null;
  data: FloorPlanData | null;
  mode: string; // Using string type to match AppStep roughly or specific canvas modes
  scaleData: ScaleData;
  ...
  onWallClick?: (wall: Wall) => void; // Specific action trigger (e.g. toggle structural)
...
const PlanCanvas: React.FC<PlanCanvasProps> = ({
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    if (mode === 'ORIENTATION' && orientation && onOrientationUpdate) {
        const cx = planWidth * 0.9;
        const cy = planHeight * 0.1;
        const radius = 60 * uiScale;
        // Check if clicking on the compass handle
    ...
    // 2. Rulers
    // Calibration Ruler
    if (mode === 'SCALE_VERIFICATION' || mode === 'SCALE_VERIFICATION_ROOMS') {
    ...
    // Garage Ruler
    if (mode === 'ORIENTATION' && garageRuler) {
    ...
    // 3. Stair Mode
    if (mode === 'STAIR_MARKING' && stairRect && onStairUpdate) {
        const handles = [
    ...
    // 4. Correction Doors
    if (mode === 'CORRECTION_DOORS' && data) {
        const clickedDoor = data.walls.find(w => w.type === 'door' && 
            Math.hypot(pos.x - (w.start.x + w.end.x)/2, pos.y - (w.start.y + w.end.y)/2) < handleRadius * 1.5
        );
        if (clickedDoor) {
            setDragMode('DOOR');
    ...
    // 6. Zone
    if (mode === 'REMODEL' || mode === 'ZONE') {
        if (remodelZone && onZoneUpdate) {
    ...
    // General Click
    if (mode === 'STRUCTURAL_ID' || mode === 'ORIENTATION' || mode === 'CORRECTION_DOORS') {
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (dragMode === 'RULER_CALIB' && activeHandle && onRulerUpdate) {
    ...
    if (dragMode === 'RULER_GARAGE' && activeHandle && onRulerUpdate && garageRuler) {
    ...
    if (dragMode === 'STAIR' && activeHandle && onStairUpdate && stairRect) {
        ...
        if (activeHandle === 'center') {
            ...
            if (activeHandle.includes('l')) { const diff = pos.x - newRect.x; newRect.x += diff; newRect.width -= diff; }
            if (activeHandle.includes('r')) { newRect.width = pos.x - newRect.x; }
            if (activeHandle.includes('t')) { const diff = pos.y - newRect.y; newRect.y += diff; newRect.height -= diff; }
            if (activeHandle.includes('b')) { newRect.height = pos.y - newRect.y; }
  ...
  if (!imageSrc) return <div className="h-full w-full bg-gray-100 flex items-center justify-center text-gray-400">Select a Floor to View</div>;
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-slate-100 border rounded-lg shadow-inner cursor-crosshair">
       <svg
        ...
        onPointerLeave={() => { setHoverPos(null); setDrawCurrent(null); }}
        preserveAspectRatio="xMidYMid meet"
        style={{ pointerEvents: 'all' }} 
      >
        {imageSrc && (
          <image href={imageSrc} x="0" y="0" width={planWidth} height={planHeight} opacity={0.5} className="pointer-events-none"/>
        )}

        {data?.walls?.map((wall, idx) => {
          const isSelected = selectedId === wall.id;
          let strokeColor = "#1e293b";
          if (mode === 'STRUCTURAL_ID') {
              strokeColor = wall.isLoadBearing ? "#dc2626" : "#cbd5e1";
              if (wall.isExternal) strokeColor = "#dc2626";
          } else {
              strokeColor = wall.isExternal ? "#ef4444" : "#1e293b";
          }
          if (isSelected) strokeColor = "#3b82f6";
          // Orientation Mode highlights front door
          if (mode === 'ORIENTATION' && orientation?.frontDoorId === wall.id) strokeColor = "#16a34a"; // Green for front door

          return (
            <g key={`wall-${idx}`}>
              <line
                x1={wall.start.x} y1={wall.start.y} x2={wall.end.x} y2={wall.end.y}
                stroke={strokeColor} strokeWidth={wall.isExternal ? wallStrokeExt : wallStroke} strokeLinecap="round"
              ...
              {wall.type === 'window' && (
               ...
               {wall.type === 'door' && (
                 <g className={mode === 'CORRECTION_DOORS' ? "cursor-move" : ""}>
                     <circle 
                        cx={(wall.start.x + wall.end.x)/2} cy={(wall.start.y + wall.end.y)/2} r={handleRadius * 0.8} 
                        fill={isSelected ? "rgba(59, 130, 246, 0.2)" : "white"} stroke={isSelected ? "#3b82f6" : "#eab308"} strokeWidth={wallStroke * 0.5} 
                     />
                     {wall.doorType && (
                         <text x={(wall.start.x + wall.end.x)/2} y={(wall.start.y + wall.end.y)/2} dy={-handleRadius} textAnchor="middle" fontSize={fontSize * 0.5} fill="#eab308" className="font-bold uppercase pointer-events-none">
                             {wall.doorType[0]} 
                         </text>
                     )}
                 </g>
        ...
        {data?.rooms?.map((room, idx) => (
            <text key={`room-${idx}`} x={room.labelPosition.x} y={room.labelPosition.y} textAnchor="middle" className="fill-slate-700 font-bold pointer-events-none select-none" style={{ fontSize: fontSize }}>
                {room.name}
            </text>
        ))}

        {remodelZone && (mode === 'ZONE' || mode === 'REMODEL') && (
          <rect x={remodelZone.x} y={remodelZone.y} width={remodelZone.width} height={remodelZone.height} fill="rgba(16, 185, 129, 0.2)" stroke="#10b981" strokeWidth={wallStroke * 0.6} strokeDasharray={`${handleRadius},${handleRadius/2}`} pointerEvents="none"/>
        )}

        {/* Stair Tool */}
        {mode === 'STAIR_MARKING' && stairRect && (
            <g>
                <rect x={stairRect.x} y={stairRect.y} width={stairRect.width} height={stairRect.height} fill="rgba(249, 115, 22, 0.2)" stroke="#f97316" strokeWidth={rulerStroke} className="cursor-move"/>
                {['tl','tr','bl','br'].map(pos => {
        ...
        {(mode === 'SCALE_VERIFICATION' || mode === 'SCALE_VERIFICATION_ROOMS') && (
            <g>
                <line x1={calibrationRuler.start.x} y1={calibrationRuler.start.y} x2={calibrationRuler.end.x} y2={calibrationRuler.end.y} stroke="#f43f5e" strokeWidth={rulerStroke} strokeDasharray={`${rulerStroke * 2},${rulerStroke}`}/>
                ...
                <text x={calibrationRuler.start.x} y={calibrationRuler.start.y - handleRadius * 1.5} textAnchor="middle" fill="#f43f5e" style={{fontSize}} className="font-bold pointer-events-none">A</text>
                ...
                <text x={calibrationRuler.end.x} y={calibrationRuler.end.y - handleRadius * 1.5} textAnchor="middle" fill="#f43f5e" style={{fontSize}} className="font-bold pointer-events-none">B</text>
            </g>
        )}

        {/* Garage Ruler (Purple) */}
        {mode === 'ORIENTATION' && garageRuler && (
            <g>
                <line x1={garageRuler.start.x} y1={garageRuler.start.y} x2={garageRuler.end.x} y2={garageRuler.end.y} stroke="#9333ea" strokeWidth={rulerStroke} strokeDasharray={`${rulerStroke * 2},${rulerStroke}`}/>
                ...
                <text x={garageRuler.start.x} y={garageRuler.start.y - handleRadius * 1.5} textAnchor="middle" fill="#9333ea" style={{fontSize}} className="font-bold pointer-events-none">G1</text>
                ...
                <text x={garageRuler.end.x} y={garageRuler.end.y - handleRadius * 1.5} textAnchor="middle" fill="#9333ea" style={{fontSize}} className="font-bold pointer-events-none">G2</text>
            </g>
        )}
        ...
        {mode === 'ORIENTATION' && orientation && (
             <g transform={`translate(${planWidth * 0.9}, ${planHeight * 0.1})`}>
                 ...
                 <text y={85 * uiScale} textAnchor="middle" className="font-bold text-xs pointer-events-none" style={{fontSize: fontSize * 0.8}} fill="#64748b">FRONT</text>
             </g>
        )}
